...
func (c *ciphers) encipher(lr []uint, off int) {
	l := lr[off] ^ c.P[0]
	r := lr[off+1]
 ...
 */
func (c *ciphers) ekskey(data []byte, key []byte) {
	var word uint
	koff := 0
	doff := 0
	lr := []uint{0, 0}
...
func crypt_raw(password []byte, salt []byte, log_rounds uint) []byte {
	c := &ciphers{P:p_orig, S:s_orig, data:bf_crypt_ciphertext}
...
type chain struct {
	...
	// errStart at this level at the next call to As long as
...
// `\uFFFD`. To return early with an error instead, useChain to
...
type replacementDecoder struct{ NopResetter }
...
type replacementEncoder struct{ NopResetter }
...
func (e *SimpleEncoding) NewDecoder() *Decoder {
	return &Decoder{Transformer: e.Decoder}
}

func (e *SimpleEncoding) NewEncoder() *Encoder {
	return &Encoder{Transformer: e.Encoder}
}
